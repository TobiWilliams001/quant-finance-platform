...
if run_optimization and symbols:
    try:
        with st.spinner("Fetching data and optimizing portfolio..."):
            ...
            if len(valid_symbols) < 2:
                st.error("At least 2 valid symbols are required for portfolio optimization.")
                st.stop()
            ...
            if returns_data.empty or len(returns_data) < 50:
                st.error("Insufficient data for optimization. Need at least 50 observations.")
                st.stop()
            ...
            with col1:
                ...
                st.metric("Expected Return", f"{sharpe_metrics['expected_return']*100:.2f}%")
            ...
            with col2:
                ...
                st.metric("Expected Return", f"{min_vol_metrics['expected_return']*100:.2f}%")
            ...
            with col2:
                ...
                weights_df_min = weights_df_min[weights_df_min['Weight (%)'] > 0.1].sort_values('Weight (%)', ascending=False)
            ...
            portfolio_variance = np.dot(weights_max_sharpe.T, np.dot(optimizer.cov_matrix, weights_max_sharpe))
            marginal_contrib = np.dot(optimizer.cov_matrix, weights_max_sharpe) / np.sqrt(portfolio_variance)
            ...
            risk_contrib_df = risk_contrib_df[risk_contrib_df['Weight (%)'] > 0.1].sort_values('Risk Contribution (%)', ascending=False)
            ...
            random_returns = np.random.normal(portfolio_mean, portfolio_std, (n_simulations, time_horizon))
            ...
            for t in range(1, time_horizon + 1):
                portfolio_values[:, t] = portfolio_values[:, t-1] * (1 + random_returns[:, t-1])
            ...
            with col1:
                if st.button("📋 Generate PDF Report"):
                    with st.spinner("Generating PDF report..."):
                        ...
                        pdf_bytes = report_generator.generate_portfolio_optimization_report(
                            frontier_data,
                            portfolio_data,
            ...
            with col2:
                if st.button("📊 Export Data to CSV"):
                    ...
                    csv_files = report_generator.export_data_to_csv(csv_data, "portfolio_optimization")
    ...
    if st.button("📊 View Sample Efficient Frontier"):
        ...
        sample_assets = ['Conservative Bond', 'Balanced Fund', 'Growth Stock', 'Dividend Stock']
        ...
        st.caption("This is a sample efficient frontier showing the concept. Use real data for actual optimization.")
