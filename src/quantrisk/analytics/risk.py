...
class RiskAnalytics:
    ...
    def calculate_max_drawdown(self, prices: pd.Series) -> Dict[str, float]:
        ...
        # Find drawdown trough index
        max_dd_trough = drawdown.idxmin()

        # Find drawdown start (peak before the trough)
        max_dd_start = cumulative[:max_dd_trough].idxmax()

        # Find drawdown end (first time it hits peak again after the trough)
        peak_value = running_max[max_dd_trough]
        after_trough = cumulative[max_dd_trough:]
        recovery = after_trough[after_trough >= peak_value]

        if not recovery.empty:
            max_dd_end = recovery.index[0]
        else:
            max_dd_end = cumulative.index[-1]

        return {
            "max_drawdown": max_drawdown,
            "max_drawdown_start": max_dd_start,
            "max_drawdown_trough": max_dd_trough,
    ...
    def calculate_sortino_ratio(
        self, returns: pd.Series, risk_free_rate: float = 0.02, target_return: float = 0
    ) -> float:
        """
    ...
    def comprehensive_risk_report(
        ...
        if benchmark_returns is not None:
            ...
            report["tracking_error"] = self.calculate_tracking_error(
