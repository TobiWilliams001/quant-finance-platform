...
class BlackScholesModel:
    ...
    def implied_volatility(
        ...
        try:
            ...
            return (
                result.x if result.success else 0.20
            )  # Default 20% if optimization fails
        except Exception:
            return 0.20

    def option_payoff(
        self, spot_prices: np.array, K: float, option_type: str = "call"
    ) -> np.array:
        """
    ...
    def plot_option_payoff(
        self, S: float, K: float, option_type: str = "call"
    ) -> go.Figure:
        """
        ...
        # Strike price line
        fig.add_vline(
            x=K, line_dash="dash", line_color="red", annotation_text=f"Strike: ${K:.2f}"
        )
    ...
    def plot_greeks_sensitivity(
        ...
        for row in [1, 1, 2, 2]:
            for col in [1, 2, 1, 2]:
                fig.add_vline(
                    x=S, line_dash="dash", line_color="gray", row=row, col=col
                )
