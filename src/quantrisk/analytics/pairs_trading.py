...
class PairsTrading:
    ...
    def calculate_hedge_ratio(self, y: pd.Series, x: pd.Series) -> Dict:
        ...
        try:
            ...
            se_beta = (
                np.sqrt(mse / np.sum((x_aligned.flatten() - np.mean(x_aligned)) ** 2))
                if np.sum((x_aligned.flatten() - np.mean(x_aligned)) ** 2) != 0
                else 0
            )
    ...
    def generate_trading_signals(
        ...
        # Generate signals using numpy for speed and to avoid SettingWithCopyWarning
        z_vals = zscore.values
        pos = np.zeros(len(z_vals))
        long_sigs = np.zeros(len(z_vals))
        short_sigs = np.zeros(len(z_vals))
        ...
        for i in range(1, len(z_vals)):
            ...
            # Exit logic
                ...
                if abs(curr_z) < exit_threshold:
                    current_pos = 0

            pos[i] = current_pos

        signals["long_signal"] = long_sigs
        signals["short_signal"] = short_sigs
        ...
        signals["strategy_returns"] = (
            signals["position"].shift(1) * signals["spread_returns"] * -1
        )
        ...
        sharpe_ratio = (
            ...
            * np.sqrt(252)
            if signals["strategy_returns"].std() != 0
            else 0
        )
        ...
        return {
            ...
            "win_rate": (
                len(signals[signals["strategy_returns"] > 0])
                / len(signals[signals["strategy_returns"] != 0])
                if len(signals[signals["strategy_returns"] != 0]) > 0
                else 0
            ),
        }
    ...
    def plot_pairs_analysis(
        ...
        if not coint_result.get("cointegrated", False):
            ...
            fig = make_subplots(
                ...
                cols=1,
                subplot_titles=[
                    f"{stock1_name} vs {stock2_name} Price Comparison",
            ...
            fig.update_layout(title="Pairs Analysis (Not Cointegrated)", height=600)

            return fig

        # Full analysis for cointegrated pairs
        hedge_result = self.calculate_hedge_ratio(
            coint_result["y_series"], coint_result["x_series"]
        )
        ...
        # Add regression line
        x_range = np.linspace(
            hedge_result["x_aligned"].min(), hedge_result["x_aligned"].max(), 100
        )
        ...
        # Add mean line
        fig.add_hline(
            y=spread.mean(), line_dash="dash", line_color="gray", row=3, col=1
        )
        ...
        for threshold in [2, -2, 0.5, -0.5]:
            ...
            fig.add_hline(
                y=threshold, line_dash=line_dash, line_color=line_color, row=4, col=1
            )
    ...
    def pairs_summary_table(
        self, stock1_data: pd.Series, stock2_data: pd.Series
    ) -> Dict:
        """
                ...
                summary.update(
                    {
                        ...
                        "spread_stationarity_p": spread_stationarity.get(
                            "p_value", np.nan
                        ),
                        "spread_is_stationary": spread_stationarity.get(
                            "is_stationary", False
                        ),
