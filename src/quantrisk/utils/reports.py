...
class ReportGenerator:
    ...
    def plotly_to_image(
        self, fig: go.Figure, width: int = 800, height: int = 600
    ) -> str:
        """
    ...
    def create_summary_table(
        self, data: Dict[str, Any], title: str = "Summary Statistics"
    ) -> Table:
        """
    ...
    def generate_risk_analytics_report(
        self, analysis_results: Dict, portfolio_name: str = "Portfolio"
    ) -> bytes:
        """
        ...
        if "returns_mean" in analysis_results:
            detailed_metrics["Annual Return"] = (
                f"{analysis_results['returns_mean']*100:.2f}%"
            )
        if "returns_std" in analysis_results:
            detailed_metrics["Annual Volatility"] = (
                f"{analysis_results['returns_std']*100:.2f}%"
            )
        ...
        if "sortino_ratio" in analysis_results:
            detailed_metrics["Sortino Ratio"] = (
                f"{analysis_results['sortino_ratio']:.4f}"
            )
        if "var" in analysis_results:
            detailed_metrics["VaR (95%)"] = (
                f"{analysis_results['var'].get(0.95, 0)*100:.2f}%"
            )
            detailed_metrics["VaR (99%)"] = (
                f"{analysis_results['var'].get(0.99, 0)*100:.2f}%"
            )
        if "cvar" in analysis_results:
            detailed_metrics["CVaR (95%)"] = (
                f"{analysis_results['cvar'].get(0.95, 0)*100:.2f}%"
            )
            detailed_metrics["CVaR (99%)"] = (
                f"{analysis_results['cvar'].get(0.99, 0)*100:.2f}%"
            )
        if "max_drawdown" in analysis_results:
            detailed_metrics["Maximum Drawdown"] = (
                f"{abs(analysis_results['max_drawdown'])*100:.2f}%"
            )
        if "beta" in analysis_results:
            detailed_metrics["Beta"] = f"{analysis_results['beta']:.4f}"
        if "tracking_error" in analysis_results:
            detailed_metrics["Tracking Error"] = (
                f"{analysis_results['tracking_error']*100:.2f}%"
            )
    ...
    def generate_portfolio_optimization_report(
        ...
        portfolio_data: Dict,
        portfolio_name: str = "Optimized Portfolio",
    ) -> bytes:
        """
        ...
        # Title
        title = Paragraph(
            f"Portfolio Optimization Report - {portfolio_name}", self.title_style
        )
        story.append(title)
        story.append(Spacer(1, 20))
        ...
        if "max_sharpe" in optimization_results:
            ...
            summary_table = self.create_summary_table(
                summary_metrics, "Optimal Portfolio Metrics"
            )
        ...
        recommendations_heading = Paragraph(
            "Investment Recommendations", self.heading_style
        )
    ...
    def generate_options_pricing_report(
        self, option_data: Dict, option_type: str = "call"
    ) -> bytes:
        """
        ...
        # Title
        title = Paragraph(
            f"Options Pricing Analysis - {option_type.capitalize()} Option",
            self.title_style,
        )
        story.append(title)
        story.append(Spacer(1, 20))
    ...
    def export_data_to_csv(
        self, data: Dict[str, pd.DataFrame], filename_prefix: str = "financial_data"
    ) -> Dict[str, bytes]:
        """
