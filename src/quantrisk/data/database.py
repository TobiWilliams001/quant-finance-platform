...
class AnalysisHistory(Base):
    ...
    )  # 'risk', 'portfolio', 'options', 'monte_carlo', 'pairs'
...
class DatabaseManager:
    ...
    def _initialize_connection(self):
        ...
        try:
            ...
            if not database_url:
                logger.warning(
                    "No DATABASE_URL found. Database features will be disabled."
                )
                return
    ...
    def get_session(self):
        """Get a database session (deprecated: use session_scope instead)"""
        if not self.initialized:
            return None
    ...
    def get_analysis_history(
        self, analysis_type: str = None, limit: int = 50
    ) -> List[Dict]:
        ...
        try:
            with self.session_scope() as session:
                ...
                history = (
                    query.order_by(AnalysisHistory.created_at.desc()).limit(limit).all()
                )
        ...
        try:
            with self.session_scope() as session:
                if session:
                    ...
                    returns_json = (
                        returns_data.to_json(date_format="iso")
    ...
    def get_portfolio_snapshots(self, limit: int = 20) -> List[Dict]:
        ...
        try:
            with self.session_scope() as session:
                ...
                snapshots = (
                    session.query(PortfolioSnapshots)
                    ...
                    results.append(
                        {
                            ...
                            "risk_metrics": (
                                json.loads(snapshot.risk_metrics)
        ...
        try:
            with self.session_scope() as session:
                if session:
                    ...
                    data_json = (
                        data.to_json(date_format="iso") if not data.empty else "{}"
                    )
                    expires_at = datetime.utcnow().replace(
                        microsecond=0
                    ) + pd.Timedelta(minutes=expires_minutes)
    ...
    def get_cached_market_data(
        self, symbol: str, period: str, data_type: str = "price"
        ...
        try:
            with self.session_scope() as session:
                ...
                stats["successful_analyses"] = (
                    ...
                    .filter(AnalysisHistory.success == True)
                    .count()
                )
                ...
                recent_analyses = (
                    ...
                    .filter(
                        ...
                        >= datetime.utcnow() - pd.Timedelta(days=7)
                    )
                    .count()
                )
