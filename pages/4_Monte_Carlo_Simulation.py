...
if simulation_type == "Stock Price Simulation":
    ...
    symbol = st.sidebar.text_input(
        "Stock Symbol", value="AAPL", help="Enter stock symbol for analysis"
    ).upper()
    ...
    symbols = [
        s.strip().upper() for s in symbols_input.strip().split("\n") if s.strip()
    ]
    ...
    underlying_symbol = st.sidebar.text_input("Underlying Symbol", value="AAPL").upper()
    ...
    K = st.sidebar.number_input(
        "Strike Price ($)", min_value=0.01, value=105.0, step=0.01, format="%.2f"
    )
    ...
    sigma = (
        st.sidebar.number_input(
            "Volatility (%)", min_value=1.0, max_value=200.0, value=25.0, step=1.0
        )
        / 100
    )
    ...
    S0 = st.sidebar.number_input(
        "Initial Price ($)", min_value=0.01, value=100.0, step=0.01
    )

    mu = (
        st.sidebar.number_input(
            "Drift (%)", min_value=-50.0, max_value=50.0, value=10.0, step=1.0
        )
        / 100
    )
    ...
    jump_intensity = st.sidebar.number_input(
        "Jump Intensity (per year)", min_value=0.0, max_value=50.0, value=5.0, step=0.5
    )

    jump_mean = (
        st.sidebar.number_input(
            "Jump Mean (%)", min_value=-50.0, max_value=50.0, value=-5.0, step=1.0
        )
        / 100
    )
    ...
    jump_std = (
        st.sidebar.number_input(
            "Jump Std Dev (%)", min_value=1.0, max_value=100.0, value=15.0, step=1.0
        )
        / 100
    )
    ...
    S0 = st.sidebar.number_input(
        "Initial Value", min_value=0.01, value=100.0, step=0.01
    )

    theta = st.sidebar.number_input(
        "Mean Reversion Speed", min_value=0.1, max_value=10.0, value=2.0, step=0.1
    )

    mu_lr = st.sidebar.number_input(
        "Long-term Mean", min_value=0.01, value=100.0, step=0.01
    )

    sigma_mr = st.sidebar.number_input(
        "Volatility", min_value=0.01, value=20.0, step=0.01
    )
...
if simulation_type == "VaR Estimation":
    ...
    confidence_levels = st.sidebar.multiselect(
        "Confidence Levels", [0.90, 0.95, 0.99], default=[0.95, 0.99]
    )
...
if run_simulation:
        ...
        with st.spinner("Running Monte Carlo simulation..."):
            ...
            if simulation_type == "Stock Price Simulation":
                ...
                price_fig = mc_simulator.plot_price_simulation(
                    ...
                    T,
                    (
                        f"{symbol} Stock Price Simulation"
                        if symbol
                        else "Stock Price Simulation"
                    ),
                )
                ...
                if len(valid_symbols) < 2:
                    st.error(
                        "At least 2 valid symbols required for portfolio simulation"
                    )
                    st.stop()
                ...
                portfolio_fig = mc_simulator.plot_price_simulation(
                    portfolio_results["portfolio_values"],
                    initial_value,
                    T,
                ...
                return_dist_fig = mc_simulator.plot_return_distribution(
                    portfolio_results["total_returns"], "Portfolio Return Distribution"
                )
                ...
                convergence_fig = mc_simulator.convergence_analysis(
                    convergence_prices, sim_sizes
                )
                ...
                with col1:
                    ...
                    st.metric(
                        "Skewness",
                        f"{np.mean((jump_final - np.mean(jump_final))**3) / np.std(jump_final)**3:.3f}",
                    )
                ...
                with col2:
                    ...
                    st.metric(
                        "Skewness",
                        f"{np.mean((gbm_final - np.mean(gbm_final))**3) / np.std(gbm_final)**3:.3f}",
                    )
                ...
                fig_comparison.add_trace(
                    go.Histogram(
                        x=jump_final, name="Jump Diffusion", opacity=0.7, nbinsx=30
                    ),
                    row=2,
                    col=1,
                )
                ...
                fig_comparison.add_trace(
                    go.Histogram(x=gbm_final, name="GBM", opacity=0.7, nbinsx=30),
                    row=2,
                    col=1,
                )

                fig_comparison.update_layout(
                    title="Jump Diffusion vs Geometric Brownian Motion", height=800
                )
            ...
            with col1:
                if st.button("📊 Export Data to CSV"):
                    ...
                    if simulation_type == "Stock Price Simulation":
                        ...
                        csv_data["final_prices"] = pd.DataFrame(
                        ...
                        csv_data["final_prices"] = pd.DataFrame(
                            {"Final_Stock_Price": option_results["final_prices"]}
                        )
                        csv_data["payoffs"] = pd.DataFrame(
                            {"Option_Payoff": option_results["payoffs"]}
                        )

                    csv_files = report_generator.export_data_to_csv(
                        csv_data, "monte_carlo_simulation"
                    )
    ...
    if st.button("🎲 View Sample Simulation"):
        ...
        st.caption(
            "This is a sample simulation with S₀=$100, μ=10%, σ=20%, T=1 year, 1000 simulations"
        )
