...
volatility_input = st.sidebar.selectbox(
    "Volatility Source", ["Manual Input", "Historical Volatility"], index=0
)
...
if calculate_option:
    try:
        ...
        with col1:
            st.metric("Black-Scholes Price", f"${bs_price:.4f}", delta=None)

        with col2:
            st.metric("Intrinsic Value", f"${intrinsic_value:.4f}", delta=None)

        with col3:
            st.metric("Time Value", f"${time_value:.4f}", delta=None)

        with col4:
            st.metric("Moneyness", moneyness, delta=None)
        ...
        if show_payoff:
            ...
            payoff_fig = bs_model.plot_option_payoff(
            ...
            if st.button("📊 Export Data to CSV"):
                csv_data = {
                    ...
                    "volatility_sensitivity": pd.DataFrame(
                        {"Volatility": vol_range, "Option_Price": vol_prices}
                    ),
                    "rate_sensitivity": pd.DataFrame(
                        {"Interest_Rate": rate_range, "Option_Price": rate_prices}
                    ),
                }

                csv_files = report_generator.export_data_to_csv(
                    csv_data, "options_pricing"
                )
    ...
    if st.button("🧮 View Sample Calculation"):
        ...
        **Results:**
        ...
        st.caption(
            "This is a sample calculation. Use the sidebar to input your own parameters."
        )
