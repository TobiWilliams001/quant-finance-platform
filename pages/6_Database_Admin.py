...
with tab1:
    ...
    if analysis_history:
        ...
        daily_counts = (
            history_df.groupby(["date", "analysis_type"])
            .size()
        ...
        recent_display["symbols"] = recent_display["symbols"].apply(
            lambda x: ", ".join(x) if isinstance(x, list) else str(x)
        )
        recent_display["created_at"] = recent_display["created_at"].dt.strftime(
            "%Y-%m-%d %H:%M"
        )
...
with tab2:
    ...
    if portfolio_snapshots:
        ...
        portfolio_display["symbols"] = portfolio_display["symbols"].apply(
            lambda x: ", ".join(x) if isinstance(x, list) else str(x)
        )
        portfolio_display["created_at"] = portfolio_display["created_at"].dt.strftime(
            "%Y-%m-%d %H:%M"
        )
        ...
        st.info(
            "No portfolio snapshots found. Save some portfolio analyses to see data here."
        )
...
with tab3:
    ...
    if db_manager.initialized:
        ...
        cache_stats = {
            ...
            "Expired Entries": stats.get("cache_entries", 0)
            - stats.get("active_cache", 0),
        }
