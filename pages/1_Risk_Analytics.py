...
if use_sample_data and "sample_period" in st.session_state:
    default_period_index = ["1y", "2y", "3y", "5y", "max"].index(
        st.session_state.sample_period
    )
else:
...
confidence_levels = st.sidebar.multiselect(
    "VaR Confidence Levels", [0.90, 0.95, 0.99], default=default_confidence
)
...
if use_sample_data and "sample_benchmark" in st.session_state:
    ...
    default_benchmark_index = (
        benchmark_options.index(st.session_state.sample_benchmark)
        if st.session_state.sample_benchmark in benchmark_options
        else 1
    )
else:
    ...
    try:
        with st.spinner("Fetching market data and performing risk analysis..."):
            ...
            portfolio_weights = np.array(
                [weights.get(symbol, 0) for symbol in returns_data.columns]
            )
            ...
            if benchmark_symbol != "None":
                try:
                    with st.spinner(f"Fetching benchmark data ({benchmark_symbol})..."):
                        benchmark_data = data_fetcher.fetch_stock_data(
                            benchmark_symbol, period
                        )
                    ...
                    if not benchmark_data.empty and len(benchmark_data) > 10:
                        ...
                        if len(benchmark_returns) > 10:
                            ...
                            if len(aligned_data) > 10:
                                ...
                                st.success(
                                    f"✅ Benchmark data ({benchmark_symbol}) loaded successfully!"
                                )
                            else:
                                benchmark_returns = None
                                st.warning(
                                    f"⚠️ Insufficient overlapping data between portfolio and benchmark ({benchmark_symbol}). Proceeding without benchmark comparison."
                                )
                        else:
                            benchmark_returns = None
                            st.warning(
                                f"⚠️ Insufficient benchmark data points for {benchmark_symbol}. Proceeding without benchmark comparison."
                            )
                    else:
                        benchmark_returns = None
                        st.warning(
                            f"⚠️ Unable to fetch sufficient benchmark data for {benchmark_symbol}. Proceeding without benchmark comparison."
                        )

                except Exception as e:
                    benchmark_returns = None
                    st.warning(
                        f"⚠️ Error fetching benchmark data for {benchmark_symbol}: {str(e)}. Proceeding without benchmark comparison."
                    )
            ...
            if db_manager.initialized:
                try:
                    ...
                    db_manager.save_portfolio_snapshot(
                        ...
                        benchmark=(
                            benchmark_symbol if benchmark_symbol != "None" else None
                        ),
                        period=period,
                    )
                    ...
                    st.warning(
                        f"⚠️ Analysis completed but database save failed: {str(db_error)}"
                    )
            ...
            with col1:
                ...
                st.metric("Annual Return", f"{annual_return:.2f}%", delta=None)

            with col2:
                annual_vol = risk_report["returns_std"] * 100
                st.metric("Annual Volatility", f"{annual_vol:.2f}%", delta=None)
            ...
            with col4:
                ...
                st.metric("Max Drawdown", f"{max_dd:.2f}%", delta=None)
            ...
            x_range = np.linspace(
                portfolio_returns.min() * 100, portfolio_returns.max() * 100, 100
            )
            normal_dist = (1 / (returns_std * np.sqrt(2 * np.pi))) * np.exp(
                -0.5 * ((x_range - returns_mean) / returns_std) ** 2
            )
            ...
            fig_dist.update_layout(
                title="Portfolio Return Distribution Analysis", height=400
            )
            ...
            with col1:
                    ...
                    with st.spinner("Generating PDF report..."):
                        try:
                            ...
                            st.session_state.pdf_bytes = (
                                report_generator.generate_risk_analytics_report(
                ...
                if st.session_state.pdf_generated and hasattr(
                    st.session_state, "pdf_bytes"
                ):
                    st.download_button(
            ...
            with col2:
                ...
                if st.button("📊 Export Data to CSV", key="generate_csv_btn"):
                    with st.spinner("Preparing CSV export..."):
                        try:
                                ...
                                "portfolio_prices": pd.DataFrame(
                                    {
                                        ...
                                        "prices": portfolio_prices.values,
                                    }
                                ),
                            ...
                            if benchmark_returns is not None:
                                csv_data["benchmark_returns"] = pd.DataFrame(
                                    {
                ...
                if st.session_state.csv_generated and hasattr(
                    st.session_state, "csv_files"
                ):
                    for i, (filename, csv_bytes) in enumerate(
                        st.session_state.csv_files.items()
                    ):
                        display_name = filename.split("_")[
                            2:-2
                        ]  # Extract meaningful part of filename
                        display_name = (
                            " ".join(display_name).title() if display_name else filename
                        )
                        st.download_button(
