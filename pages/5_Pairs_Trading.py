...
if selection_method == "Manual Selection":
    stock1 = st.sidebar.text_input(
        "Stock 1 Symbol", value="AAPL", help="Enter the first stock symbol"
    ).upper()

    stock2 = st.sidebar.text_input(
        "Stock 2 Symbol", value="MSFT", help="Enter the second stock symbol"
    ).upper()
    ...
    selected_sector = st.sidebar.selectbox("Select Sector", list(sector_pairs.keys()))

    pair_options = [f"{pair[0]} vs {pair[1]}" for pair in sector_pairs[selected_sector]]
    selected_pair_str = st.sidebar.selectbox("Select Pair", pair_options)
    ...
    selected_pair_str = st.sidebar.selectbox("Select Popular Pair", pair_options)
...
if run_analysis and stock1 and stock2:
    try:
        with st.spinner(f"Analyzing {stock1} vs {stock2} pair..."):
            ...
            if show_cointegration:
                ...
                coint_result = pairs_analyzer.cointegration_test(
                    stock1_prices, stock2_prices
                )

                if coint_result.get("cointegrated", False):
                    st.success(
                        "✅ Stocks are cointegrated - suitable for pairs trading!"
                    )
                    ...
                    with col1:
                        st.metric(
                            "Test Statistic", f"{coint_result['test_statistic']:.4f}"
                        )
                    ...
                    **Interpretation:** The Engle-Granger test indicates cointegration between {stock1} and {stock2}.
                    ...
                    st.warning(
                        "⚠️ Stocks are NOT cointegrated - pairs trading may not be effective"
                    )

                    if "error" in coint_result:
                        st.error(
                            f"Error in cointegration test: {coint_result['error']}"
                        )
                    else:
                        st.info(f"""
            ...
            hedge_result = pairs_analyzer.calculate_hedge_ratio(
                stock1_prices, stock2_prices
            )
            ...
            if "error" not in hedge_result:
                ...
                with col1:
                    ...
                    st.caption(
                        f"Long 1 share of {stock1}, Short {abs(hedge_ratio):.4f} shares of {stock2}"
                    )
                ...
                spread = pairs_analyzer.calculate_spread(
                    stock1_prices, stock2_prices, hedge_ratio
                )
                ...
                if not zscore.empty:
                    ...
                    if abs(current_zscore) >= entry_threshold:
                        if current_zscore > 0:
                            signal_text = (
                                f"🔴 **SELL SIGNAL** - Z-score: {current_zscore:.2f}"
                            )
                            signal_interpretation = f"Short {stock1}, Long {stock2} (spread is overextended)"
                        else:
                            signal_text = (
                                f"🟢 **BUY SIGNAL** - Z-score: {current_zscore:.2f}"
                            )
                        ...
                        st.info(
                            f"🔵 **EXIT SIGNAL** - Z-score: {current_zscore:.2f} (close to equilibrium)"
                        )

                    else:
                        st.info(
                            f"⚪ **NO SIGNAL** - Z-score: {current_zscore:.2f} (waiting for entry threshold)"
                        )
            ...
            if show_trading_signals and "error" not in hedge_result:
                ...
                if "signals" in signals_result:
                    ...
                    fig_signals.add_hline(
                        y=0, line_dash="solid", line_color="gray", row=1, col=1
                    )
                    ...
                    if not long_signals.empty:
                        fig_signals.add_trace(
                            go.Scatter(
                                ...
                                name="Long Entry",
                                marker=dict(
                                    color="green", size=10, symbol="triangle-up"
                                ),
                            ),
                            row=1,
                            col=1,
                    ...
                    if not short_signals.empty:
                        fig_signals.add_trace(
                            go.Scatter(
                                ...
                                name="Short Entry",
                                marker=dict(
                                    color="red", size=10, symbol="triangle-down"
                                ),
                            ),
                            row=1,
                            col=1,
            ...
            if "error" not in hedge_result:
                ...
                with col1:
                    ...
                    if "error" not in stationarity_result:
                        ...
                        if is_stationary:
                            st.success(
                                f"✅ Spread is stationary (p-value: {adf_pvalue:.6f})"
                            )
                        else:
                            st.warning(
                                f"⚠️ Spread may not be stationary (p-value: {adf_pvalue:.6f})"
                            )

                        st.metric(
                            "ADF Statistic",
                            f"{stationarity_result['adf_statistic']:.4f}",
                        )
            ...
            aligned_returns = pd.concat(
                [stock1_returns, stock2_returns], axis=1
            ).dropna()
            ...
            if len(aligned_returns) > 0:
                    ...
                    performance_data.append(
                        {
                            ...
                            "Correlation": (
                                f"{aligned_returns[stock1].corr(aligned_returns[stock2]):.4f}"
                                if symbol == stock1
                                else "-"
                            ),
                        }
                ...
                rolling_corr = (
                    aligned_returns[stock1]
                    .rolling(correlation_window)
                    .corr(aligned_returns[stock2])
                )
            ...
            summary_data = pairs_analyzer.pairs_summary_table(
                stock1_prices, stock2_prices
            )
            ...
            with col1:
                ...
                if summary_data.get("cointegrated", False):
                    ...
                    st.metric(
                        ...
                        f"{summary_data.get('cointegration_p_value', 0):.6f}",
                    )
                    st.metric(
                        "Hedge Ratio", f"{summary_data.get('hedge_ratio', 0):.4f}"
                    )
                    ...
                    st.metric(
                        "Cointegration P-Value",
                        f"{summary_data.get('cointegration_p_value', 1):.6f}",
                    )
            ...
            with col1:
                if st.button("📊 Export Data to CSV"):
                    csv_data = {
                            ...
                            [
                                {
                                    ...
                                    "Cointegrated": summary_data.get(
                                        "cointegrated", False
                                    ),
                                    "Hedge_Ratio": summary_data.get(
                                        "hedge_ratio", np.nan
                                    ),
                        ...
                        "stock1_prices": pd.DataFrame(
                            {"Date": stock1_prices.index, "Price": stock1_prices.values}
                        ),
                        "stock2_prices": pd.DataFrame(
                            {"Date": stock2_prices.index, "Price": stock2_prices.values}
                        ),
            ...
            with col2:
                st.info(
                    "PDF report generation can be added for detailed pairs analysis"
                )
    ...
    if st.button("📊 View Sample Analysis"):
        ...
        stock_b = (
            50
            + np.cumsum(np.random.normal(0.0008, 0.015, len(dates)))
            + 0.3 * (stock_a - 100)
        )
        ...
        fig_sample.update_layout(
            title="Sample Pairs Trading Analysis", height=800, showlegend=True
        )

        st.plotly_chart(fig_sample, use_container_width=True)

        st.caption(
            "This is a sample visualization showing pairs trading concepts. Use real market data for actual analysis."
        )
